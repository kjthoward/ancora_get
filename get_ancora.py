...
for link in soup.findAll('a'):
    ...
    if species_name!="/":
        ...
        print(f"{f'Collecting list of {species_name.removesuffix("/")} _vs_ files':-^60}")
        for link in (species_soup.findAll("a")):
            ...
            print(f"{f'Downloading files for {species_name.removesuffix("/")}_{vs_name.removesuffix("/")}':-^60}")
            for vs_link_name in vs_soup.findAll("a"):
                vs_link=vs_link_name.get("href")
                if vs_link.endswith(".bed") or vs_link.endswith(".wig") or vs_link.endswith(".bed.gz") or vs_link.endswith(".wig.gz"):
                    print(f"Downloading: {vs_link}")
                    open(os.path.join(outdir,vs_link),"wb").write(requests.get(vs_url+"/"+vs_link).content)
